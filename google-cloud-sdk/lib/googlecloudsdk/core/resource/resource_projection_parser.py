  ...
  def _ParseKeyAttributes(self, key, attribute):
    ...
    while True:
      ...
      if not self._lex.IsCharacter('=', eoi_ok=True):
        raise SyntaxError('name=value expected [{0}].'.format(
      ...
      if name == 'alias':
        ...
        if value not in resource_projection_spec.ALIGNMENTS:
          raise SyntaxError('Unnown alignment [{0}].'.format(
              self._lex.Annotate(here)))
        ...
        raise SyntaxError('Unnown key attribute [{0}].'.format(
            self._lex.Annotate(here)))
  ...
  def _ParseKeys(self):
    ...
    while True:
      ...
      if self._lex.IsCharacter(')'):
        break
      if not self._lex.IsCharacter(','):
        raise SyntaxError('Expected ) in projection expression [{0}].'.format(
            self._lex.Annotate()))
  ...
  def _ParseAttributes(self):
    ...
    while True:
      ...
      if self._lex.IsCharacter(']'):
        break
      if not self._lex.IsCharacter(','):
        raise SyntaxError('Expected ] in attribute list [{0}].'.format(
            self._lex.Annotate()))
