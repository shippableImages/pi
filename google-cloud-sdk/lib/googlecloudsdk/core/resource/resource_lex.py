...
Typical resource usage:
  ...
  while lex.SkipSpace():
    ...
    if not key:
      ...
      raise SyntaxError('key expected [%s].' % lex.Annotate(here))
    ...
    if not operator:
      ...
      if lex.IsString('AND'):
        ...
        raise SyntaxError('Operator expected [%s].' % lex.Annotate())
    ...
    if not operand:
      raise SyntaxError('Operand expected [%s].' % lex.Annotate())
...
class Lexer(object):
  ...
  def Annotate(self, position=None):
    """Returns the expression string annotated for SyntaxError messages.
  ...
  def SkipSpace(self, token=None):
    ...
    Raises:
      SyntaxError: End of input reached after skipping.
    ...
    while not self.EndOfInput():
      ...
      self._position += 1
    if token:
      raise SyntaxError('{0} expected [{1}].'.format(token, self.Annotate()))
    return False
  ...
  def IsCharacter(self, characters, peek=False, eoi_ok=False):
    ...
    Raises:
      SyntaxError: End of input reached and peek is False.
    ...
    if self.EndOfInput():
      if peek or eoi_ok:
        return None
      raise SyntaxError('More tokens expected [%s].' % self.Annotate())
  ...
  def Args(self, convert=False):
    ...
    while True:
      ...
      if arg is not None:
        ...
        raise SyntaxError('Argument expected [%s].' % self.Annotate(here))
      if end:
        break
      if not self.IsCharacter(','):
        raise SyntaxError('Closing ) expected in argument list [%s].' %
                          self.Annotate(here))
      required = True
    return args
  ...
  def Key(self):
    ...
    while not self.EndOfInput():
      ...
      if name:
        ...
        raise SyntaxError('Non-empty key name expected [%s].' %
                          self.Annotate(here))
      if self.EndOfInput():
        break
      if self.IsCharacter(']'):
        raise SyntaxError('Unmatched ] in key [%s].' % self.Annotate(here))
      if self.IsCharacter('['):
      ...
      if not self.IsCharacter('.'):
        break
      if self.EndOfInput():
        raise SyntaxError('Non-empty key name expected [%s].' % self.Annotate())
    return key
