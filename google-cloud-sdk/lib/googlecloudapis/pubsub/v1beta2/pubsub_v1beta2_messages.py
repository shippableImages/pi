...
class Rule(_messages.Message):
  ...
  Fields:
    ...
    notIn: The rule matches if the PRINCIPAL/AUTHORITY_SELECTOR is not in this
      set of entries. The formation for in and not_in entries is the same as
      members in a Binding above.
...
class Subscription(_messages.Message):
  ...
  Fields:
    ackDeadlineSeconds: This value is the maximum time after a subscriber
      ...
      It may be overridden for each message using its corresponding ack_id by
      calling ModifyAckDeadline.  For push delivery, this value is also used
