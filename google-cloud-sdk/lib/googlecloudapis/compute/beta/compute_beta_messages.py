...
class AttachedDisk(_messages.Message):
  ...
  Fields:
    ...
    deviceName: Specifies a unique device name of your choice that is
      ...
      disks.
    diskEncryptionKey: Specifies the customer-supplied encryption key to
      protect the disk.  When attempting to use a disk that is protected by a
      customer-supplied encryption key, you must provide the correct key. If
      you provide an incorrect key, or no key, the operation will fail.
      Customer-supplied encryption keys do not protect access to metadata of
      the disk.  If no customer-supplied encryption key is provided at
...
class AttachedDiskInitializeParams(_messages.Message):
  ...
  Fields:
    ...
    sourceImageEncryptionKey: Specifies the customer-supplied encryption key
      of the source image. This key is required if the source image is
...
class Backend(_messages.Message):
  ...
  Fields:
    ...
    capacityScaler: The multiplier (a value between 0.0 and 1.0) of the max
...
class BackendService(_messages.Message):
  ...
  together with their serving capacity.  If you add field foo, you probably
  need to also add:
  ...
  BackendServiceEntity: getFoo, setFoo:  Converters/mappers will need to be
  updated: com.google.cloud.cluster.manager.networking.services.backendservice
  .BackendServiceResourceConverter: toResource, updateEntity: copy foo com.goo
  gle.cloud.cluster.mixer.protomappers.BackendServiceMappers.ResourceMapper:
  ResourceMapper: add a new map call  Tests to update: com.google.cloud.cluste
  r.manager.networking.services.backendservice.BackendServiceResourceConverter
  Test com.google.cloud.cluster.mixer.protomappers.BackendServiceMappersTest.t
  estResourceMapping
...
class CustomerEncryptionKey(_messages.Message):
  ...
  Fields:
    rawKey: Specifies the customer-supplied encryption key to protect the data
      stored within the resource.  To access a resource that is protected by a
      customer-supplied encryption key, you must provide the correct key that
      matches the encryption key. Note that customer-supplied encryption keys
      do not restrict access to any metadata about the resource.  If no
      customer-supplied encryption key is provided at creation, then the
      resource will be encrypted using an automatically generated key.
      Format: 256-bit key material encoded in Base 64 (RFC4648).
    rsaEncryptedKey: Specifies the customer-supplied encryption key to protect
      the data stored within the resource.  Use this field if you choose to
      encrypt your customer-supplied encryption key using the RSA public key
      provided by Google.  To access a resource that is protected by a
      customer-supplied encryption key, you must provide the correct key that
      matches the encryption key. Note that customer-supplied encryption keys
      do not restrict access to any metadata about the resource.  If no
      customer-supplied encryption key is provided at creation, then the
      resource will be encrypted using an automatically generated key.
      Format: 256-bit key material RSA encrypted and encoded in Base 64
      (RFC4648).
    sha256: [Output only] The SHA-256 hash of the customer-supplied encryption
...
class Disk(_messages.Message):
  ...
  Fields:
    ...
    sourceSnapshotEncryptionKey: Specifies the customer-supplied encryption
      key of the source snapshot. This key is required if the source snapshot
      is protected by a customer-supplied encryption key.  If the incorrect
...
class Image(_messages.Message):
  ...
  Fields:
    ...
    id: [Output Only] Unique identifier for the resource; defined by the
      server.
    imageEncryptionKey: Specifies the customer-supplied encryption key to
      protect the image.  When attempting to use a image that is protected by
      a customer-supplied encryption key, you must provide the correct key. If
      you provide an incorrect key, or no key, the operation will fail.
      Customer-supplied encryption keys do not protect access to metadata of
      the image.  If no customer-supplied encryption key is provided at
      creation, then the image will be encrypted using an automatically
      generated key.
...
class Snapshot(_messages.Message):
  ...
  Fields:
    ...
    name: Name of the resource; provided by the client when the resource is
      created. The name must be 1-63 characters long, and comply with RFC1035.
    selfLink: Server defined URL for the resource (output only).
    snapshotEncryptionKey: Specifies the customer-supplied encryption key to
      protect the snapshot.  When attempting to use a snapshot that is
      protected by a customer-supplied encryption key, you must provide the
      correct key. If you provide an incorrect key, or no key, the operation
      will fail.  Customer-supplied encryption keys do not protect access to
      metadata of the snapshot.  If no customer-supplied encryption key is
      provided at creation, then the image will be encrypted using an
