...
name and kind already exists.
    ...
    def Delete(self, request, global_params=None):
      """Deletes the alias with the given name and kind. Kind cannot be ANY.
If the alias does not exist, NOT_FOUND is returned.
If the request provides a revision ID and the alias does not
refer to that revision, ABORTED is returned.
...
workspace's current snapshot ID.
...
the most recent update to the workspace.
...
A refresh may involve merging files in the workspace with files in the current
alias revision. If this merge results in conflicts, then the workspace is
in a merge state: the merge_info field of Workspace will be populated,
and conflicting files in the workspace will contain conflict markers.
    ...
    def ResolveFiles(self, request, global_params=None):
      """Marks files modified as part of a merge as having been resolved.
    ...
    def RevertRefresh(self, request, global_params=None):
      """If a call to RefreshWorkspace results in conflicts, use RevertRefresh.
to restore the workspace to the state it was in before the refresh.
Returns FAILED_PRECONDITION if not preceded by a call to RefreshWorkspace,
or if there are no unresolved conflicts remaining.
...
response list is sorted by name with the default repo listed first.
    ...
    def Merge(self, request, global_params=None):
      """Merges a revision into a movable alias, using a workspace associated.
with that alias to store modified files. The workspace must not have any modified
files. Note that Merge neither creates the workspace nor commits it;
those actions must be done separately.
